...
namespace BICYCL
{
 ...
 private:
  ...
  {
  public:
   CL_HSMqk_DL_CL_ZKProof(const OpenSSL::ECGroup &E, const CL_HSMqk_DL_CL_ZKProof &p);

   CL_HSMqk_DL_CL_ZKProof(const CL_HSMqk &C,
                          const OpenSSL::ECGroup &E,
                          OpenSSL::HashAlgo &H,
   ...
   bool verify(const CL_HSMqk &C, const OpenSSL::ECGroup &E, OpenSSL::HashAlgo &H,
               const OpenSSL::ECPoint &X,
               const CL_HSMqk::CipherText &c0,
 ...
 {
 public:
  CL_HSMqk_Part_Dec_ZKProof(const CL_HSMqk &C, OpenSSL::HashAlgo &H,
                           const CL_HSMqk::PublicKey &pk,
                           const CL_HSMqk::CipherText &c,
                           const QFI &pd,
                           const CL_HSMqk::SecretKey &sk,
                           RandGen &randgen);
