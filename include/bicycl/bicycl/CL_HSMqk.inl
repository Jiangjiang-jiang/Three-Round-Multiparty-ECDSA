...
inline
CL_HSMqk_PolyVerify_ZKProof::CL_HSMqk_PolyVerify_ZKProof(const CL_HSMqk &C, const OpenSSL::ECGroup &E,
                              OpenSSL::HashAlgo &H,
...
inline
bool CL_HSMqk_PolyVerify_ZKProof::verify(const CL_HSMqk &C,
...
inline
Mpz CL_HSMqk_Part_Dec_ZKProof::k_from_hash(OpenSSL::HashAlgo &H, const CL_HSMqk::PublicKey &pk,
                       const CL_HSMqk::CipherText &c,
                       const QFI &pd,
                       const QFI &t1, const QFI &t2)
{
  return Mpz (H(c, pk, pd, t1, t2));
}

inline
CL_HSMqk_Part_Dec_ZKProof::CL_HSMqk_Part_Dec_ZKProof(const CL_HSMqk &C, OpenSSL::HashAlgo &H,
                                                     const CL_HSMqk::PublicKey &pk,
                                                     const CL_HSMqk::CipherText &c,
                                                     const QFI &pd,
                                                     const CL_HSMqk::SecretKey &sk,  RandGen &randgen)
{
  int soundness = H.digest_nbits();
...
inline
bool CL_HSMqk_Part_Dec_ZKProof::verify(const CL_HSMqk &C, OpenSSL::HashAlgo &H,
                           const CL_HSMqk::PublicKey &pk,
                           const CL_HSMqk::CipherText &c,
                           const QFI &pd) const
{
  ...
  QFI eql_1, eqr_1;
  C.power_of_h(eql_1, z_);

  pk.exponentiation (C, eqr_1, k_);
  if (C.compact_variant())
    C.from_Cl_DeltaK_to_Cl_Delta (eqr_1);
  C.Cl_G().nucomp(eqr_1, eqr_1, t1_);

  b &= (eql_1 == eqr_1);

  QFI eql_2, eqr_2;

  C.Cl_G().nupow(eql_2, c.c1(), z_);
  C.Cl_G().nupow(eqr_2, pd, k_);
  C.Cl_G().nucomp(eqr_2, eqr_2, t2_);
...
inline
CL_HSMqk_DL_CL_ZKProof::CL_HSMqk_DL_CL_ZKProof(const OpenSSL::ECGroup &E, const CL_HSMqk_DL_CL_ZKProof &p)
  : z1_(p.z1_), k_(p.k_), t1_(p.t1_), t2_(p.t2_), s_(E, p.s_)
{
}

inline
CL_HSMqk_DL_CL_ZKProof::CL_HSMqk_DL_CL_ZKProof(const CL_HSMqk &C,
                           const  OpenSSL::ECGroup &E,
                           OpenSSL::HashAlgo &H,
                           const OpenSSL::ECPoint &X,
                           const CL_HSMqk::CipherText &c0,
                           const CL_HSMqk::CipherText &c1,
                           const CL_HSMqk::ClearText &x,
                           RandGen &randgen) : s_(E)
{
  int soundness = H.digest_nbits();
  ...
  Mpz a (randgen.random_mpz (B));
  E.scal_mul_gen(s_, OpenSSL::BN(a));

  // E.mul_mod_order(z2_, OpenSSL::BN(k_), OpenSSL::BN(x));
  // E.add_mod_order(z2_, z2_, OpenSSL::BN(a));

  C.Cl_G().nupow (t1_, c0.c1(), a);
  C.Cl_Delta().nupow (t2_, c0.c2(), a);

  k_ = k_from_hash(H, X, c0, c1, t1_, t2_);
  Mpz::mul (z1_, k_, x);
  Mpz::add (z1_, z1_, a);
...
inline
Mpz CL_HSMqk_DL_CL_ZKProof::k_from_hash (OpenSSL::HashAlgo &H,
                    const OpenSSL::ECPoint &X,
                    const CL_HSMqk::CipherText &c1,
                    const CL_HSMqk::CipherText &c2,
                    const QFI &t1, const QFI &t2)
{
  return Mpz (H (c1, c2, t1, t2));
}

inline
bool CL_HSMqk_DL_CL_ZKProof::verify(const CL_HSMqk &C, const OpenSSL::ECGroup &E,
               OpenSSL::HashAlgo &H,
               const OpenSSL::ECPoint &X,
               const CL_HSMqk::CipherText &c0,
               const CL_HSMqk::CipherText &c1) const
{
  ...
  QFI eql_1, eqr_1, eql_2, eqr_2;

  C.Cl_G().nupow (eql_1, c0.c1(), z1_);
  C.Cl_G().nupow (eqr_1, c1.c1(), k_);
  C.Cl_G().nucomp(eqr_1, eqr_1, t1_);

  ret &= (eql_1 == eqr_1);

  C.Cl_Delta().nupow (eql_2, c0.c2(), z1_);
  C.Cl_Delta().nupow (eqr_2, c1.c2(), k_);
  C.Cl_Delta().nucomp(eqr_2, eqr_2, t2_);

  ret &= (eql_2 == eqr_2);

  OpenSSL::ECPoint eql_3(E, OpenSSL::BN(z1_));
  OpenSSL::ECPoint eqr_3(E, X);
  E.scal_mul(eqr_3, OpenSSL::BN(k_), eqr_3);
  E.ec_add(eqr_3, eqr_3, s_);

  ret &= E.ec_point_eq(eql_3, eqr_3);
