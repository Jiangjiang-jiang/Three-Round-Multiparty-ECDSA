...
{
public:
    Party(GroupParams& params, unsigned int id, const CL_HSMqk::PublicKey& pk, const std::vector<CL_HSMqk::PublicKey>& pki_v, const CL_HSMqk::SecretKey& ski, const OpenSSL::ECPoint& X, std::vector<OpenSSL::ECPoint> &X_v, const OpenSSL::BN& xi)
    ...
    RoundOneData& getRoundOneData() const {
        if (!round1Data) {
    ...
    std::tuple<Commitment, CommitmentSecret> commit(const OpenSSL::ECPoint &Q)
    {
        size_t nbytes = static_cast<unsigned int>(SecLevel::_128) >> 3;
    ...
    bool open(const Commitment &c, const OpenSSL::ECPoint &Q, const CommitmentSecret &r)
    {
    ...
    void handleRoundOne() {
        ...
        round1Data = std::make_unique<RoundOneData>(params.ec_group, id, enc_phi_share, com_i, zk_proof_cl_enc);
    ...
    void handleRoundThree(std::vector<RoundTwoData>& data, std::vector<unsigned char>& m) {
        ...
        for (unsigned int i = 1; i < params.t+1; ++i)
        {
    ...
    void handleOffline(std::vector<RoundThreeData>& data)
    {
        ...
        std::unordered_map<unsigned int, QFI> part_c0_dec_shares;
        std::unordered_map<unsigned int, QFI> part_c1_dec_shares;

        for(unsigned int i = 0; i < params.t+1; ++i)
        {
            part_c0_dec_shares[data[i].id] = data[i].c0_dec_share;
            part_c1_dec_shares[data[i].id] = data[i].c1_dec_share;
        }

        CL_HSMqk::ClearText m0 = agg_partial_decryptions(part_c0_dec_shares, round3LocalData->c0);
        CL_HSMqk::ClearText m1 = agg_partial_decryptions(part_c1_dec_shares, round3LocalData->c1);
    ...
    bool verify(Signature& signature, std::vector<unsigned char>& m)
    {
        OpenSSL::BN h (params.H(m));
        OpenSSL::BN inv_s;
        OpenSSL::BN u1, u2;
...
private:
    ...
    CL_HSMqk::ClearText agg_partial_decryptions(std::unordered_map<unsigned int, QFI>& decryptions, CL_HSMqk::CipherText &c)
    {
        QFI c2 = c.c2();
        ...
        {
            QFI num;
            params.cl_pp.Cl_G().nupow (num, decryptions[s + 1], cl_lagrange_at_zero(params.cl_pp, S, s, delta));
